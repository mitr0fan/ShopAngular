...
export class AsideComponent implements OnInit {
  ...
  p2 = [
    ['По алфавиту', 'По цене - по возрастанию', 'По цене - по убыванию'],
    ['Forward', 'Format', 'Altair'],
    [],
    ...
    [
      ...
      'Передняя амортизационная вилка и задний амортизатор',
    ],
  ...
  ngOnInit() {
    if (this.getBicycles.previousArr == undefined) {
      ...
      this.bicycles = this.getBicycles.previousArr;
    }
    this.giveBicycles();
  }
  ...
  check(checkDiv?: HTMLElement) {
    this.bicycles = [];
    const items: NodeListOf<HTMLElement> = document.body.querySelectorAll(
      '.item'
    );
    ...
    if (checkDiv != undefined) {
      ...
      if (checkBox.style.backgroundColor == '') {
        if (elem.slice(0, elem.length - 1) != 'Сортировка') {
          ...
          const sort: NodeListOf<HTMLElement> = checkDiv.parentElement.querySelectorAll(
            '.checkBox'
          );
    ...
    for (let i = 0; i < items.length; i++) {
      const checkArr: NodeListOf<HTMLElement> = items[i].querySelectorAll(
        '.checkBox'
      );
      let sum = 0;
    ...
    const sortCheck: NodeListOf<HTMLElement> = document.body
      .querySelector('.item')
  ...
  move(e, elem) {
    ...
    const onePx = Math.ceil(
      (this.max - this.min) / (line.offsetWidth - elem.offsetWidth)
    );
    if (elem.id == 'point1') {
      let value =
        Math.ceil(
          elem.getBoundingClientRect().x - line.getBoundingClientRect().x
        ) *
          onePx +
        this.min;
      if (value > this.max) {
